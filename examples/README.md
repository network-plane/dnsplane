...
| File | Purpose |
| --- | --- |
| [dnsplane-example.json](dnsplane-example.json) | Starter `dnsplane.json` (DoT / DoH / DNSSEC keys present, off by default). Includes **`dashboard_resolution_log_cap`** (default `1000`) for the in-memory dashboard **Resolutions log**, and optional **`cluster_*`** keys (`cluster_sync_policy`, `cluster_discovery_srv`, etc.; see [docs/clustering.md](../docs/clustering.md)). |
