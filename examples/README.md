...
| [curl-stats-dashboard.sh](curl-stats-dashboard.sh) | **`curl`** calls for `GET /stats/dashboard`, `/stats/dashboard/data`, and **`/stats/dashboard/resolutions`**. Set `BASE` (default `http://127.0.0.1:8080`). Add `Authorization: Bearer …` if `api_auth_token` is set. |
